# -*- coding: utf-8 -*-
"""
Created on Sat Nov 30 19:14:34 2019
...
#SOLVABLE GRID CREATOR 

def creategrid(rows, cols, bombs, rpos, cpos):
    # Input: no. of rows (int), cols (int), bombs (int), rpos and cpos of first click
    # Output: SOLVABLE grid with specified rows, cols, bombs (array) + bomb location grid
...
def border(winstate):
    #Input: an array with 1's for opened, 0's for unopened, -1's for flagged
    #Output: list of unopened/flagged tiles on the border
    winstate
    
grid,wincond = creategrid(8,8,10)
winstate = wincond
print(winstate)
print(border(winstate))
...
def Game(rows = 8, cols = 8, bombs = 10, sqsize = 50):
    ...
    def finish(win = 1): #Update screen when gameplay ends
